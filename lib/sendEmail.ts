...
<div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;border:1px solid #e8e2df;">
  ...
  <div style="padding:40px 40px 32px;border-bottom:1px solid #ede8e5;">
    ...
    <p style="font-size:14px;color:#6b5f5a;line-height:1.75;margin:0 0 28px;">
      Your registration for ICCH-AI 2026 is confirmed. We are delighted to welcome you to the first international gathering at the intersection of contemplative science, AI, and digital health.
    </p>
    ...
    <p style="font-size:13px;color:#6b5f5a;line-height:1.75;margin:0 0 ${user.ischtMembershipNumber ? '18' : '0'}px;">
      You are now subscribed to ICCH-AI 2026 updates. We will send the online session link before the event.
    </p>
    ...
    <div style="background:#F7F4F2;border:1px solid #e8e2df;border-top:3px solid #C69232;border-radius:6px;padding:20px 24px;">
      ...
      <div style="font-size:13px;color:#1a0a0a;line-height:1.7;margin-bottom:10px;">You're in as a founding member of the International Society for Contemplative Technology. Your membership card (PDF) is attached separately.</div>
  ...
  const attachments = [{
    filename: `ICCH-AI-2026-Pass-${user.registrationNumber}.pdf`,
    content: b64,
  }]
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'ICCH-AI 2026 <registrations@icchai.com>',
      reply_to: 'ceodhyanly@gmail.com',
      to: user.email,
      subject: `Registration Confirmed — ICCH-AI 2026 (${user.registrationNumber})`,
      html,
      attachments,
    }),
  })
...
<div style="max-width:480px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;border:1px solid #e8e2df;">
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      reply_to: 'ceodhyanly@gmail.com',
      to: email,
      subject: `Your ICCH-AI 2026 verification code: ${code}`,
      html,
    }),
  })
